...
class BiDirectionalAttention(nn.Module):
    ...
    def __init__(self, sem_dim, geo_dim, inter_dim=126, heads=6):
        super().__init__()
        # 1. 维度对齐
        self.proj_sem = nn.Conv2d(sem_dim, inter_dim, kernel_size=1)
        self.proj_geo = nn.Conv2d(geo_dim, inter_dim, kernel_size=1)
        
        # 2. X-Attention机制
        self.attn_sem_to_geo = Attention(query_dim=inter_dim, context_dim=inter_dim, heads=heads, dim_head=inter_dim//heads)
        self.attn_geo_to_sem = Attention(query_dim=inter_dim, context_dim=inter_dim, heads=heads, dim_head=inter_dim//heads)
        
        self.norm_sem = nn.LayerNorm(inter_dim)
        self.norm_geo = nn.LayerNorm(inter_dim)

    def forward(self, sem, geo):
        B, _, H, W = sem.shape
        
        sem_proj = self.proj_sem(sem) 
        geo_proj = self.proj_geo(geo) 
        
        # 铺平满足 Attention 维度需求 [B, H*W, inter_dim]
        sem_flat = sem_proj.flatten(2).permute(0, 2, 1) 
        geo_flat = geo_proj.flatten(2).permute(0, 2, 1)
        
        sem_out = self.norm_sem(sem_flat + self.attn_sem_to_geo(sem_flat, context=geo_flat))
        geo_out = self.norm_geo(geo_flat + self.attn_geo_to_sem(geo_flat, context=sem_flat))
        
        # 将相互渗透后的特征拼接
        fused = torch.cat([sem_out, geo_out], dim=-1)
        fused = fused.permute(0, 2, 1).reshape(B, 2 * sem_proj.shape[1], H, W)
...
class RotaryPositionEncoding3D(RotaryPositionEncoding):
    # 这是一个非常特殊的 3D RoPE 实现。与常见的 NLP 一维 RoPE 或 图像二维 RoPE 不同，这里的 RoPE 是直接基于 3D 世界坐标 (World XYZ) 进行编码的。
    # 步骤一：提取真实的 3D 坐标 (Source of Position)
    # 步骤二：计算 3D 旋转频率 (Frequency Computation)
    # 步骤三：注入图像 Token (Injection)；RoPE 不是像传统 PE 那样“加”上去的，而是通过旋转操作“乘”上去的。
    ...
    def __init__(
        ...
        self.gate_fuse_layers = nn.ModuleList([
            GateFuser(
                ...
                in_ch_da=geo_channels[i],
                mid_ch=mid_channels[i])
            for i in range(len(geo_channels))
        ])

        self.bida_res3 = BiDirectionalAttention(sem_dim=sem_channels[2], geo_dim=geo_channels[2], inter_dim=fpn_fuse_dim)
        self.bida_res4 = BiDirectionalAttention(sem_dim=sem_channels[3], geo_dim=geo_channels[3], inter_dim=fpn_fuse_dim)
        self.bida_res5 = BiDirectionalAttention(sem_dim=sem_channels[4], geo_dim=geo_channels[4], inter_dim=fpn_fuse_dim)

        self.post_bida_fpn = FeaturePyramidNetwork(
            in_channels_list=[2 * fpn_fuse_dim, 2 * fpn_fuse_dim, 2 * fpn_fuse_dim],
            out_channels=fpn_fuse_dim
        )

        self.downsample_conv = nn.Sequential(
            nn.Conv2d(fpn_fuse_dim, fpn_fuse_dim, kernel_size=3, stride=2, padding=1),
            nn.GroupNorm(1, fpn_fuse_dim),
    ...
    def forward(
        ...
        with torch.no_grad(): # 提取深度专家特征
            ...
            depth_expert_feat = depth_expert.get_intermediate_layers(
                x=rgb_de,
                n=layer_ids_de,
                reshape=True, #############3 vit架构从切块到恢复图像维度
                return_class_token=False, ### 丢弃位置编码
                norm=True
            )
        ...
        # 在 res3, res4, res5 层做双向注意力
        fused_res3 = self.bida_res3(semantic_feat['res3'], sgm_feat['res3'])
        fused_res4 = self.bida_res4(semantic_feat['res4'], sgm_feat['res4'])
        fused_res5 = self.bida_res5(semantic_feat['res5'], sgm_feat['res5'])
        ...
        # 将结果拼接并通过 FPN，取 res3 输出
        fpn_out = self.post_bida_fpn(fused_dict)
...
class SpatialActor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.sem_enc_type = sem_enc_type # 语义编码器类型（如 'CLIP-RN50'）
        self.geo_enc_type = geo_enc_type # 几何编码器类型（如 'RN50'）
        self.dep_exp_type = dep_exp_type # 深度专家（Depth Anything）类型

        self.lang_dim = lang_dim
        self.lang_len = lang_len
        self.add_proprio = add_proprio
        self.proprio_dim = proprio_dim # 机器人本体感知（关节状态等）的原始维度
        self.proprio_cat_dim = proprio_cat_dim # 和映射后的拼接维度。

        self.spt_view_layers = spt_view_layers # 视图级（View-Level） 交互层数
        self.spt_scene_layers = spt_scene_layers # 场景级（Scene-Level） 交互层数
        self.im_channels = im_channels # 核心 Transformer 处理的中间特征通道数
        ...
        ########## 核心模块初始化流程：双流编码器，语义引导几何模块，投影与对齐，空间 Transformer 主干，解码头

        # learnable positional encoding # 位置编码
        ...
        ##### 不仅仅简单拼接 RGB 和 Depth 特征，而是用“深度专家”（Depth Anything）提取的强力深度特征，
        # 通过门控机制（GateFuser） 来融合到几何流中。
        ...
        self.multimodal_fusion = SemanticGuidedGeometricModule(
        ...
        # 应用 3D 旋转位置编码 (RotaryPositionEncoding3D)：这是为了让网络理解 3D 空间结构的关键，特别是处理多视图点云渲染的特征时。
        # 进行两阶段 Attention：View Interact: 视图内部或局部特征交互；Scene Interact: 全局交互（融合语言、所有视图）
        self.spatial_transformer = SpatialTransformer( # 模型主干
        ...
        self.trans_head = ConvexUpSample( # 上采样获取热力图
        ...
        if not self.no_feat:
            ...
            # 基于 feat_ver 提取特征（通常是取热力图峰值处的特征）
            # 使用 MLP (fc_head) 预测具体的动作参数：旋转 (Rotation) 和其他参数。这里采用了自回归方式预测 X->Y->Z 轴旋转
    ...
    def forward(
        ...
        bs, num_img, img_feat_dim, h, w = img.shape # 渲染好的多视角 RGB-D 图像特征。在 RVT 类架构中，点云已经被渲染成了多个视角（如 Front, Left, Right 等）的 Feature Map
        # 通常包含了 RGB (3) + 深度 (1) + 坐标信息 (3) 等
        assert num_img == self.num_img
        assert h == w == self.img_size

        # preprocess
        # (bs * num_img, im_channels, h, w)
        img = img.view(bs * num_img, img_feat_dim, h, w) # 批次大小 乘以 渲染视角的数量
        d0 = self.input_preprocess(img) # 图像特征维度（根据你的配置，通常是 7 或 10，取决于是否加入像素位置编码
        # 通道 0~2：点云对应的 3D 坐标信息 ( XYZ )
        # 通道 3~5：RGB 语义与纹理信息 ( Color )
        # 通道 6：深度信息 ( Depth )
        #（配置可选）通道 7~9：像素位置编码 ( Pixel Loc )

        # semantic encoder # RGB图输入
        rgb = d0[:, 3:6]
        rgb = self.rgb_norm(rgb)
        # 语义特征
        semantic_feat = self.semantic_encoder(rgb)

        # geometric encoder # 深度图输入，理解在哪里
        depth = d0[:, 6:7].clone()
        ...
        depth_normalized = (depth - depth_min) / (depth_max - depth_min + 1e-8) # ... (归一化深度图) ...
        depth_normalized = self.depth_norm(depth_normalized) 
        ###### 用 ResNet 提取几何特征
        geometic_feat = self.geometric_encoder(depth_normalized)

        # 多模态特征融合层：引入深度专家和双向交叉注意力
        spatial_feat, geometic_feat, depth_expert_feat = self.multimodal_fusion(
        ...
        # proprio projector # 本体感知投影
        if self.add_proprio:
            proprio_feat = self.proprio_proj(proprio)
            # 将本体向量广播 (Repeat) 到和图像特征一样的大小 (H, W)，拼在一起 # 意味着图上每个像素都知道机械臂现在的状态
            proprio_feat = proprio_feat.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1, 1, num_img, _h, _w)

        # spatial transformer
        # 7. 空间 Transformer (Spatial Transformer) - **决策大脑**
        # 输入: 融合后的 spatial_feat, 语言, 本体
        # 内部做了:
        # - 添加 3D 旋转位置编码 (Rotary PE)，让模型理解 3D 空间。
        # - View-Interaction: 视角内交互。
        # - Scene-Interaction: 全局交互（融合语言指令）。
        # 输出 x: 高度抽象后的特征图，包含了决策所需的所有信息。
        ...
        ######## 9. 生成热力图 (Translation Head)
        # 使用上采样层 (ConvexUpSample)，从特征图恢复出高分辨率的热力图
        trans = self.trans_head(x) # (b * num_img, 1, h, w)
        trans = trans.view(bs, self.num_img, h, w)

        # get wpt_local while testing
        # Ground Truth (GT) 关键点坐标。告诉模型“正确答案”在哪里。
        # 用于训练时的 Teacher Forcing 或者是用来从特征图中抠取局部特征训练 Action Head
        ...
        # 不仅要知道“去哪里”，还要知道“到了那个位置后怎么转”
        if not self.no_feat:
            ## 10. 确定关注点 (Focus Point)
            assert (
                0 <= wpt_img.min() and wpt_img.max() <= x.shape[-1]
            ), print(wpt_img, x.shape)

            # 11. 特征抠取 (Feature Selection)
            # 将 3D 坐标投影回 2D 图像平面，并在特征图 x 上抠出该点的特征向量
            # "既然我要去这里，那我只关心这里的特征"
            ...
            ## 12. 自回归旋转预测 (Autoregressive Rotation Prediction)
            # 预测顺序: 杂项 -> X轴 -> Y轴 -> Z轴
            ...
            ##### 将 X 的预测结果编码 (Embedding) 后加回去，辅助预测 Y
