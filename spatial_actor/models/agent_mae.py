    ...
    def update(
        ...
        if backprop:
            with autocast(enabled=self.amp):
                ...
                ######################### add_depth_mae ##########################
                mae_depth_loss = torch.tensor(0.0, device=trans_loss.device)
                if "spacial_actor2" in out and "depth_loss" in out["spacial_actor2"]:
                    _loss = out["spacial_actor2"]["depth_loss"]
                    # Weight decay: 0.1 -> 0.001
                    _progress = min(1.0, max(0.0, step / self.cos_dec_max_step))
                    _w = 0.1 - (0.1 - 0.001) * _progress
                    mae_depth_loss = _loss * _w
            ...
            loss_log = {
                ...
                "rot_loss_x": rot_loss_x.item() if isinstance(rot_loss_x, torch.Tensor) else rot_loss_x,
                "rot_loss_y": rot_loss_y.item() if isinstance(rot_loss_y, torch.Tensor) else rot_loss_y,
                "rot_loss_z": rot_loss_z.item() if isinstance(rot_loss_z, torch.Tensor) else rot_loss_z,
                "grip_loss": grip_loss.item() if isinstance(grip_loss, torch.Tensor) else grip_loss,
                "collision_loss": collision_loss.item() if isinstance(collision_loss, torch.Tensor) else collision_loss,
                "align_loss": align_loss.item() if isinstance(align_loss, torch.Tensor) else align_loss,
                "mae_depth_loss": mae_depth_loss.item() if isinstance(mae_depth_loss, torch.Tensor) else mae_depth_loss,
